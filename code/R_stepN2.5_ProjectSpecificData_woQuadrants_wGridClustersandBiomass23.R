...
projs.to.look.at <- proj.years$project.name
#now looking at all projects
...
save(list=c('proj.dats.for.prop.b2k'),
     file = "StepN2.5_ProjectSpecificData_b2k_updated.RData")
...
for (i in 1:length(projs.in.dat)) {
  ...
  if (proj.to.assess %in% disagged.projs.list) {
    print(paste0("THIS PROJECT HAS DISJOINTED BORDERS: WILL BE ASSESSED BELOW"))
